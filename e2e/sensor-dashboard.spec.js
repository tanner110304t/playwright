...
test.describe('Environmental Sensor Dashboard', function() {
  ...
  test('should display all sensor IDs on cards', async function({ page }) {
  ...
  test('should accept a boundary value of 0', async function({ page }) {
    await page.getByLabel('Sensor ID').fill('sensor-002');
    await page.getByLabel('Reading Value').fill('0');
    await page.getByRole('button', { name: 'Submit Reading' }).click();
  ...
  test('should show error message for an out-of-range value', async function({ page }) {
    await page.getByLabel('Sensor ID').fill('sensor-001');
    await page.getByLabel('Reading Value').fill('99999');
    await page.getByRole('button', { name: 'Submit Reading' }).click();

    const message = page.locator('#message-container');
    await expect(message).toHaveClass(/error-message/, { timeout: 10000 });
    await expect(message).toContainText('between -100 and 10000');
  });
  ...
  test('should show error message when only the sensor ID is filled', async function({ page }) {
    await page.getByLabel('Sensor ID').fill('sensor-001');
    await page.getByRole('button', { name: 'Submit Reading' }).click();
  ...
  test('should re-enable the submit button after a submission', async function({ page }) {
    await page.getByLabel('Sensor ID').fill('sensor-001');
    await page.getByLabel('Reading Value').fill('44');
    const button = page.getByRole('button', { name: 'Submit Reading' });
    await button.click();

    await expect(page.locator('#message-container')).toHaveClass(/success-message/, { timeout: 10000 });
